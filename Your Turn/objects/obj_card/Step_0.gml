card_animation(target_x, target_y, spd) // animação de draw

#region Depth
var i = array_find_index(global.instances, function (value, index)
{
	if (value == id)
		return true
	return false
...
if (mouse_check_button_pressed(mb_left))
{
	...
	if (card == id)
	{
		...
		array_delete(global.instances, array_get_index(global.instances, id), 1);
		array_push(global.instances, id);
...
if (mouse_check_button_released(mb_left))
{
	if (dragging)
	{
		...
		var space = instance_position(x, y, obj_card_space)
...
if (keyboard_check(ord("R")))
{
	if (!on_slot)
	{
		...
		array_sort(global.instances, function(elm1, elm2) {
